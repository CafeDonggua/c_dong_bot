...
| 版本 | 階段目標 | 主要範圍 | 狀態 |
| --- | --- | --- | --- |
| 1.1.0 | Cron 自然語句化與相容路由 | 保留舊行程功能 + 新增自然語句 Cron 路由 | In Progress |
...
### 目前進度（2026-02-13）
- 已完成自然語句重複提醒解析與分流模組（daily/weekly/every）。
- 已完成單次事件邊界分流，避免誤建為 Cron 任務。
- 已補齊 `/cron help` 雙軌範例（指令式 + 自然語句）。
- 已補上對應單元與整合測試，待合併至 `next` 後做 Telegram 端到端複核。
